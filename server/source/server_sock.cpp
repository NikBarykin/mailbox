...
namespace Socket {
    ...
    Listener::Listener(std::string servname, SocketArgs sock_args,
        ...
        if (i_result != 0) {
            throw SocketError("bind failed: " + std::to_string(i_result));
        }
        i_result = listen(sock_, backlog);
        if (i_result != 0) {
            throw SocketError("listen failed: " + std::to_string(i_result));
        }
    }
    ...
    Client Listener::Accept() {
        ...
        if (client_sock == INVALID_SOCKET) {
            throw SocketError("accept failed: " + std::to_string(WSAGetLastError()));
        }
